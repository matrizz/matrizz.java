...
client.on("interactionCreate", async (interaction) => {
  if (interaction.isButton()) {
    if (interaction.customId === "tickets_basico") {
      ...
      if (canal) {
        interaction.reply({ content: `Olá **${interaction.user.username}**, você já possui um ticket em ${canal}.`, flags: Discord.MessageFlags.Ephemeral })
      } else {
        ...
        interaction.guild.channels.create({
          ...
          interaction.reply({ content: `Olá **${interaction.user.username}**, seu ticket foi aberto em ${chat}.`, flags: Discord.MessageFlags.Ephemeral })
