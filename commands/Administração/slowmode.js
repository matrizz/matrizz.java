    ...
    run: async (client, interaction) => {
        ...
        if (!interaction.member.permissions.has(Discord.PermissionFlagsBits.ManageChannels)) {
            ...
            if (!tempo || tempo === false || tempo === null) {
                interaction.reply({ content: `Forneça um tempo válido: [s|m|h].`, flags: Discord.MessageFlags.Ephemeral })
            } else {
                channel.setRateLimitPerUser(tempo / 1000).then(() => {
                    ...
                    interaction.reply({ content: `Ops, algo deu errado ao executar este comando, verifique minhas permissões.`, flags: Discord.MessageFlags.Ephemeral })
                })
            }
